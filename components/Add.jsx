...
const Add = ({ setClose }) => {
    ...
    const handleCreate = async () => {
        ...
        try {
            const uploadRes = await axios.post("https://api.cloudinary.com/v1_1/dihfddms0/image/upload", data);
            const { url } = uploadRes.data;
            const newProduct = {
                title, desc, prices, extraOptions, img: url,
            };

            await axios.post("http://joes-pizza-delivery.vercel.app/api/products", newProduct);
            setClose(true);
        } catch (err) {
            console.log(err);
        }
        ...
        <div className={styles.container}>
            <div className={styles.wrapper}>
                ...
                <div className={styles.item}>
                    ...
                    <input className={styles.input} type="text" onChange={(e) => setDesc(e.target.value)} />
                </div>
                <div className={styles.item}>
                ...
                <div className={styles.item}>
                    ...
                    <div className={styles.extra}>
                        <input className={styles.input} type="text" placeholder="Item" name="text" onChange={handleExtraInput} />
                        <input className={styles.input} type="number" placeholder="Price" name="price" onChange={handleExtraInput} />
